...
import (
	...
	"github.com/Mueat/gotron-sdk/pkg/address"
	"github.com/Mueat/gotron-sdk/pkg/proto/core"
...
func convetToAddress(v interface{}) (eCommon.Address, error) {
	switch vs := v.(type) {
	case string:
		addr, err := address.Base58ToAddress(vs)
		if err != nil {
			return eCommon.Address{}, fmt.Errorf("invalid address %s: %+v", vs, err)
		}
			...
			if ty.T == eABI.SliceTy || ty.T == eABI.ArrayTy {
				if ty.Elem.T == eABI.AddressTy {
					...
					for _, tmpv := range tmp {
						adrs = append(adrs, tmpv.(string))
					}
					addresses := make([]eCommon.Address, 0)
					for aidx := range adrs {
						addr, err := convetToAddress(adrs[aidx])
						if err != nil {
							return nil, err
						}
				...
				if (ty.Elem.T == eABI.IntTy || ty.Elem.T == eABI.UintTy) &&
					...
					for _, s := range v.([]interface{}) {
						...
						if strings.HasPrefix(ss, "0x") {
							value, _ = new(big.Int).SetString(ss[2:], 16)
						} else {
							value, _ = new(big.Int).SetString(ss, 10)
						}
